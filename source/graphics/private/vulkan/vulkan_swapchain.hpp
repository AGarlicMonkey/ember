...
namespace ember::inline graphics {
    ...
    private:
        ...
        array<unique_ptr<vulkan_image>> images{};

        //FUNCTIONS
    public:
        vulkan_swapchain() = delete;
        vulkan_swapchain(VkInstance instance, VkDevice device, VkSurfaceKHR surface, VkSwapchainKHR handle, VkFormat swapchain_image_format, VkExtent2D swapchain_extent, array<unique_ptr<vulkan_image>> images);
        ...
        array<image *> get_images() const override;

        inline VkSwapchainKHR get_handle() const;

        static result convert_result(VkResult const result);

        static VkSurfaceFormatKHR choose_surface_format(array<VkSurfaceFormatKHR> const &available_formats);
        static VkPresentModeKHR choose_present_mode(array<VkPresentModeKHR> const &available_present_modes);
